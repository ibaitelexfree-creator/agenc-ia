...
import { perExamQuestions, ExamQuestion } from './data';
import { Question, DetailedResult, EvaluationResult } from '@/components/academy/evaluation/types';
        ...
        <div className="min-h-screen bg-nautical-black py-20 px-6">
            <div className="max-w-5xl mx-auto space-y-8">
                ...
                <div className="bg-white/5 backdrop-blur-xl rounded-3xl border border-white/10 p-12 text-center relative overflow-hidden">
                    <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-96 h-96 blur-[120px] pointer-events-none opacity-20 ${isPassing ? 'bg-green-500' : 'bg-red-500'}`} />

                    <div className="relative z-10">
                        <div className={`w-32 h-32 rounded-full flex items-center justify-center mx-auto mb-8 text-6xl shadow-2xl ${
                            isPassing ? 'bg-green-500/20 text-green-400 ring-1 ring-green-500/50' : 'bg-red-500/20 text-red-400 ring-1 ring-red-500/50'
                        }`}>
                            {isPassing ? '🏆' : '⚠️'}
                        </div>
                        ...
                        <div className="flex items-baseline justify-center gap-2 mb-8">
                            <span className={`text-7xl font-black tracking-tighter ${isPassing ? 'text-green-400' : 'text-red-400'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {blockScores.map((block) => (
                        <div key={block.category} className="bg-white/5 border border-white/10 p-6 rounded-2xl flex flex-col gap-4">
                            <div className="flex justify-between items-start">
                                <h3 className="font-bold text-white text-sm uppercase tracking-wide h-10 flex items-center">
                                    {block.category}
                                </h3>
                                <div className={`px-2 py-1 rounded text-xs font-bold ${
                                    block.percentage >= 70 ? 'bg-green-500/20 text-green-400' :
                                    block.percentage >= 50 ? 'bg-yellow-500/20 text-yellow-400' :
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between text-xs text-slate-400">
                                    <span>Aciertos</span>
                                    <span className="text-white font-mono">{block.correct} / {block.total}</span>
                                </div>
                                <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                                    <div
                                        className={`h-full rounded-full transition-all duration-1000 ${
                                            block.percentage >= 70 ? 'bg-green-500' :
                                            block.percentage >= 50 ? 'bg-yellow-500' :
                                            'bg-red-500'
                                        }`}
                    ...
                    <div className="space-y-4 animate-in slide-in-from-top-4 duration-500">
                        {result.details.map((detail, idx) => (
                            <div key={idx} className={`p-6 rounded-2xl border ${
                                detail.isCorrect ? 'bg-green-500/5 border-green-500/20' : 'bg-red-500/5 border-red-500/20'
                            }`}>
                                <div className="flex gap-4">
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ${
                                        detail.isCorrect ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
                                    }`}>
                                        {idx + 1}
                                    </div>
                                    <div className="flex-1 space-y-3">
                                        <p className="font-medium text-white text-lg">{detail.enunciado_es}</p>

                                        <div className="grid md:grid-cols-2 gap-4 text-sm">
                                            <div className="space-y-1">
                                                <span className="text-slate-500 uppercase text-xs font-bold">Tu Respuesta</span>
                                                <div className={`p-3 rounded-lg border ${
                                                    detail.isCorrect ? 'bg-green-500/10 border-green-500/30 text-green-300' : 'bg-red-500/10 border-red-500/30 text-red-300'
                                                }`}>
                                            ...
                                            {!detail.isCorrect && (
                                                <div className="space-y-1">
                                                    <span className="text-slate-500 uppercase text-xs font-bold">Respuesta Correcta</span>
...
export default function SkillsPage() {
    ...
    const handleSubmit = () => {
        ...
        perExamQuestions.forEach(q => {
            ...
            const userOption = q.opciones_json?.find(opt => opt.id === userAnswerId);
            const correctOption = q.opciones_json?.find(opt => opt.id === q.correctAnswer);
    ...
    if (status === 'finished' && result) {
        return <PerResultScreen result={result} blockScores={blockScores} onRetry={handleStart} />;
    }
    ...
    return (
        <div className="min-h-screen bg-nautical-black">
            <div className="pt-20 px-6"> {/* Padding for header */}
                <QuizView
