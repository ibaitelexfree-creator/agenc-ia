...
export default async function RentalPage({ params: { locale } }: { params: { locale: string } }) {
    ...
    return (
        <main className="min-h-[100dvh] w-full bg-nautical-black text-sea-foam selection:bg-accent selection:text-nautical-black">
            ...
            <section className="relative pt-20 sm:pt-32 md:pt-44 pb-8 sm:pb-16 md:pb-24 overflow-hidden w-full">
                {/* Background Decor */}
                ...
                <div className="container mx-auto px-4 sm:px-6 relative z-10">
                    <header className="max-w-4xl">
                        <span className="text-accent uppercase tracking-[0.2em] sm:tracking-[0.4em] md:tracking-[0.6em] text-[10px] sm:text-xs md:text-sm font-bold mb-3 sm:mb-6 block animate-fade-in-up">
                            {t('header_eyebrow')}
                        </span>
                        <h1 className="text-[clamp(1.75rem,5.5vw,6.5rem)] font-display leading-[1.05] text-sea-foam mb-4 sm:mb-8 md:mb-12 animate-reveal relative z-20 whitespace-normal sm:whitespace-nowrap">
                            <span>{t('title_prefix')}</span>{' '}
                            <span className="italic font-light text-brass-gold/90 drop-shadow-sm">{t('title_highlight')}</span>
                        </h1>
                        <p className="max-w-2xl text-sea-foam/70 font-light text-xs sm:text-base md:text-xl leading-relaxed border-l-2 border-sea-foam/10 pl-3 sm:pl-6 md:pl-8 mt-4 sm:mt-8 animate-fade-in" style={{ animationDelay: '0.8s' }}>
                            {t('description')}
                        </p>
                    </header>
                </div>
            ...
            <section className="pb-12 sm:pb-24 md:pb-36 relative">
                <div className="container mx-auto px-4 sm:px-6 relative z-10">
