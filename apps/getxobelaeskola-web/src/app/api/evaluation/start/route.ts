...
export async function POST(request: Request) {
    try {
        ...
        if (!evaluacion_id) {
            return NextResponse.json({ error: 'Falta evaluacion_id' }, { status: 400 });
        }
        ...
        if (accessError) {
            ...
            return NextResponse.json({ error: 'Error verificando permisos de acceso.' }, { status: 500 });
        }

        if (!tieneAcceso) {
            return NextResponse.json({
                error: 'Acceso denegado: El contenido está bloqueado o no cumples los requisitos.',
        ...
        if (evaluacion.tipo === 'quiz_unidad') {
            ...
            if (numSeccionesVistas < 3) {
                return NextResponse.json({
                    error: 'Debes completar la lectura de todas las secciones antes de realizar el quiz.',
        ...
        if (intentoExistente) {
            const TTL_HORAS = 24;
            const ultimaActividad = new Date(intentoExistente.ultima_actividad || intentoExistente.created_at);
            const horasInactivo = (now.getTime() - ultimaActividad.getTime()) / (1000 * 60 * 60);
            ...
            if (horasInactivo > TTL_HORAS) {
                ...
                const ordenMap = new Map((intentoExistente.preguntas_json as string[]).map((id: string, i: number) => [id, i]));
                const preguntasOrdenadas = preguntas.sort((a: { id: string }, b: { id: string }) =>
                ...
                return NextResponse.json({
                    ...
                    tiempo_inicio: intentoExistente.fecha_inicio || intentoExistente.created_at
                });
            }
        }
        ...
        const ultimoIntento = ultimoIntentoData && ultimoIntentoData.length > 0 ? ultimoIntentoData[0] : null;

        // A. COOLDOWN
        const cooldownMin = evaluacion.cooldown_minutos ?? 0;

        if (cooldownMin > 0 && ultimoIntento) {
            const lastTimeStr = ultimoIntento.fecha_completado || ultimoIntento.created_at;
        ...
        if (ventanaLimite && ventanaHoras) {
            ...
            if (countVentanaError) {
                return NextResponse.json({ error: countVentanaError.message }, { status: 500 });
            }

            if (countVentana !== null && countVentana >= ventanaLimite) {
                const { data: intentosVentanaData, error: intentosVentanaError } = await supabase
                    .from('intentos_evaluacion')
                    .select('created_at')
                    .eq('alumno_id', user.id)
                    .eq('evaluacion_id', evaluacion_id)
                ...
                if (!intentosVentanaError && intentosVentanaData && intentosVentanaData.length > 0) {
        ...
        if (evaluacion.intentos_maximos) {
            ...
            if (countTotalError) {
                return NextResponse.json({ error: countTotalError.message }, { status: 500 });
            }
        ...
        if (intentoError) {
            if (intentoError.code === '23505') {
                ...
                if (recoveryError || !intentoExistente) {
                    return NextResponse.json({ error: 'Error recuperando intento existente' }, { status: 500 });
                }

                intento = intentoExistente;

                const { data: preguntasExistentes, error: preguntasExistentesError } = await supabaseAdmin
                    .from('preguntas')
                    .select('id, tipo_pregunta, enunciado_es, enunciado_eu, opciones_json, puntos, imagen_url')
                    .in('id', intento.preguntas_json);

                if (preguntasExistentesError) {
                    return NextResponse.json({ error: preguntasExistentesError.message }, { status: 500 });
                }

                const ordenMap = new Map((intento.preguntas_json as string[]).map((id: string, i: number) => [id, i]));
                const preguntasOrdenadas = preguntasExistentes?.sort((a: { id: string }, b: { id: string }) =>
                    (ordenMap.get(a.id) ?? 0) - (ordenMap.get(b.id) ?? 0)
                );
        ...
        if (preguntasDataError) {
            return NextResponse.json({ error: preguntasDataError.message }, { status: 500 });
        }
