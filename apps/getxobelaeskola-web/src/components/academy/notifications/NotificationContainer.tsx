...
function NotificationItem({ notification, onDismiss }: { notification: Notification; onDismiss: () => void }) {
    const isSpecial = notification.type === 'achievement' || notification.type === 'skill';
        ...
        >
            ...
            <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                    <h4 className={`font-display italic font-bold tracking-tight ${notification.type === 'info' ? 'text-lg' : ''}`}>
                        {notification.title}
                    </h4>
                    <p className={`${notification.type === 'info' ? 'text-base' : 'text-sm'} text-white/60 leading-tight`}>
            ...
            {notification.duration !== 0 && (
                <div className="absolute bottom-0 left-0 h-1 bg-accent/40 w-full animate-shrink-x"
