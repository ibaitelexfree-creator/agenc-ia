...
export const TideChart: React.FC<TideChartProps> = ({ currentTime }) => {
    ...
    const pathD = chartData.reduce((acc, point, index) => {
        ...
        return index === 0 ? `M ${x.toFixed(2)} ${y.toFixed(2)}` : `${acc} L ${x.toFixed(2)} ${y.toFixed(2)}`;
    }, '');
        ...
        <div className="w-full bg-slate-900/80 rounded-xl p-3.5 sm:p-6 border border-slate-700/50 backdrop-blur-sm shadow-xl">
            ...
            <div className="relative w-full aspect-[2.5/1]">
                <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full overflow-visible" preserveAspectRatio="none">
                    ...
                    {/* Grid lines */}
                    <line x1="0" y1={height} x2={width} y2={height} stroke="#334155" strokeWidth="0.2" />
                    <line x1="0" y1={0} x2={width} y2={0} stroke="#334155" strokeWidth="0.2" />
                    <line x1="0" y1={height / 2} x2={width} y2={height / 2} stroke="#334155" strokeWidth="0.2" strokeDasharray="1 1" />

                    {/* Vertical grid lines every 6 hours */}
                    {[0, 25, 50, 75, 100].map(gx => (
                        <line key={gx} x1={gx} y1={0} x2={gx} y2={height} stroke="#334155" strokeWidth="0.1" />
                    ))}
