...
export default function RentalClient({
        ...
        try {
            ...
            if (response.status === 401) {
                sessionStorage.setItem('pendingRental', JSON.stringify({
                    serviceId, optionIndex, reservedDate: dateValue, reservedTime: selectedTime
                }));
    ...
    return (
        <div className="space-y-4 sm:space-y-8 md:space-y-12 landscape:space-y-6 pb-6 sm:pb-12 md:pb-20 landscape:pb-10 w-full max-w-full overflow-hidden">
            ...
            <motion.div 
                layout
                className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 lg:gap-10 rentals-grid-system w-full max-w-full"
                style={{
                    display: 'grid',
                    width: '100%',
                    maxWidth: '100%'
                    ...
                    <div className="relative bg-nautical-black border border-sea-foam/10 max-w-lg w-full p-8 md:p-12 animate-fade-in shadow-2xl rounded-sm">
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="flex gap-4 pt-4">
                                ...
                                <button
                                    type="button"
                                    onClick={() => handleBooking(bookingService, bookingOption !== null ? bookingOption : undefined)}
                                ...
                                >
                                    {loading ? '...' : `${t('booking.confirm')} (${(services.find(s => s.id === bookingService)?.precio_base || 0) + (bookingOption !== null ? (services.find(s => s.id === bookingService)?.opciones?.[bookingOption]?.extra || 0) : 0)}€)`}
                                </button>
                            </div>
                        </div>
                    </div>
