    ...
    const filteredBoats = React.useMemo(() => {
        ...
        result.sort((a, b) => {
            switch (sortOrder) {
                ...
                case 'capacity_desc': return b.capacidad - a.capacidad;
                case 'capacity_asc': return a.capacidad - b.capacidad;
                case 'status': return a.estado.localeCompare(b.estado);
                default: return 0;
            }
        });
        ...
        <div className="space-y-8 animate-fade-in">
            ...
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8 border-b border-white/10 pb-8">
                <div className="space-y-2 min-w-fit">
                    <span className="text-accent uppercase tracking-[0.4em] text-2xs font-bold block">Gestión de Flota</span>
                    <h2 className="text-3xl md:text-5xl font-display text-white italic whitespace-nowrap">{t('boats.title')} ({filteredBoats.length})</h2>
                </div>
                ...
                <div className="flex flex-wrap lg:flex-nowrap items-center gap-4 w-full lg:w-auto">
                    ...
                    <div className="flex flex-wrap gap-4 w-full lg:w-auto">
                        ...
                        >
                            ...
                            <option value="mantenimiento">{t('boats.status.maintenance')}</option>
                        ...
                        >
                            ...
                            <option value="vela_ligera">{t('boats.types.sailing_light')}</option>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {filteredBoats.map(boat => (
                    <div key={boat.id} className="p-6 bg-white/5 border border-white/5 rounded-sm relative group hover:border-accent/30 transition-all flex flex-col">
                        <div className="flex justify-between items-start mb-4">
                            <span className={`px-2 py-1 text-2xs uppercase tracking-widest font-bold border ${boat.estado === 'disponible' ? 'border-green-500 text-green-500' :
                                boat.estado === 'mantenimiento' ? 'border-yellow-500 text-yellow-500' :
                                    'border-red-500 text-red-500'
                                }`}>
                                {boat.estado === 'disponible' ? t('boats.status.available') :
                                    boat.estado === 'mantenimiento' ? t('boats.status.maintenance') :
                                        boat.estado === 'averiado' ? t('boats.status.broken') :
                                            t('boats.status.in_use')}
                            </span>
                            <div className="text-2xl opacity-50">
                                {boat.tipo === 'vela_ligera' ? '⛵' : boat.tipo === 'motor' ? '🛥️' : '🛶'}
                            </div>
                        </div>
                        ...
                        <div className="mt-auto space-y-4">
                            <div className="flex justify-between text-2xs text-white/40 border-t border-white/5 pt-4">
                                <span>{t('boats.capacity')}: {boat.capacidad} pax</span>
                                <span className="uppercase">{
                                    boat.tipo === 'vela_ligera' ? t('boats.types.sailing_light') :
                                        boat.tipo === 'crucero' ? t('boats.types.cruiser') :
                                            boat.tipo === 'motor' ? t('boats.types.motor') :
                                                boat.tipo === 'kayak' ? t('boats.types.kayak') :
                                                    t('boats.types.other')
                                }</span>
                            </div>
            ...
            <AccessibleModal
                ...
                title={editingBoat ? t('boats.modal.edit_title') : t('boats.modal.new_title')}
                maxWidth="max-w-2xl"
            >
                <div className="space-y-8 py-4">
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="text-2xs uppercase tracking-[0.2em] text-white/40 font-bold ml-1">{t('boats.modal.name_label')}</label>
                            <input
                                disabled={!isAdmin}
                        ...
                        <div className="space-y-2">
                            <label className="text-2xs uppercase tracking-[0.2em] text-white/40 font-bold ml-1">{t('boats.modal.plate_label')}</label>
                            <input
                                disabled={!isAdmin}
                    ...
                    <div className="grid grid-cols-3 gap-6">
                        <div className="space-y-2">
                            <label className="text-2xs uppercase tracking-[0.2em] text-white/40 font-bold ml-1">{t('boats.modal.type_label')}</label>
                            <select
                                disabled={!isAdmin}
                        ...
                        <div className="space-y-2">
                            <label className="text-2xs uppercase tracking-[0.2em] text-white/40 font-bold ml-1">{t('boats.modal.capacity_label')}</label>
                            <input
                                type="number"
                                disabled={!isAdmin}
                        ...
                        <div className="space-y-2">
                            <label className="text-2xs uppercase tracking-[0.2em] text-white/40 font-bold ml-1">{t('boats.modal.status_label')}</label>
                            <select
                                className="w-full bg-nautical-deep border border-white/10 p-4 text-white outline-none focus:border-accent"
                                value={formData.estado}
                                onChange={e => setFormData({ ...formData, estado: e.target.value })}
                            >
                                <option value="disponible" className="text-green-500">{t('boats.status.available')}</option>
                                <option value="mantenimiento" className="text-yellow-500">{t('boats.status.maintenance')}</option>
                                <option value="averiado" className="text-red-500">{t('boats.status.broken')}</option>
                                <option value="en_uso" className="text-blue-500">{t('boats.status.in_use')}</option>
                            </select>
                        </div>
                    </div>

                    <div className="space-y-2">
                        <label className="text-2xs uppercase tracking-[0.2em] text-white/40 font-bold ml-1">{t('boats.modal.notes_label')}</label>
                        <textarea
                            rows={4}
