            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-white/5 pb-8">
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] uppercase tracking-[0.4em] font-black text-indigo-400/60">Data Intelligence</span>
                    </div>
                    <h2 className="text-4xl font-display text-white italic tracking-tight">Explorador de <span className="text-indigo-400 underline underline-offset-8 decoration-indigo-400/20">Datos</span></h2>
                    <p className="text-white/40 text-sm max-w-md">Busca en toda la base de datos y visualiza conexiones entre entidades.</p>
                </div>
            </div>

            {/* Search Bar */}
            <form onSubmit={handleSearch} className="flex gap-4 p-1 bg-white/5 border border-white/10 rounded-full max-w-3xl">
                <select
                    value={selectedTable}
                ...
                >
                    {TABLES.map(t => <option key={t.id} value={t.id} className="bg-nautical-black text-white">{t.label}</option>)}
                </select>
            ...
            <div className="grid grid-cols-1 gap-4">
                {results.length > 0 ? (
                                ...
                                <div className="space-y-1">
                                    <div className="flex items-center gap-3">
                                        <span className="text-[10px] uppercase font-black tracking-widest text-white/20 bg-white/5 px-2 py-0.5 rounded-sm">
                                            {TABLES.find(t => t.id === result._table)?.label || result._table}
                                        </span>
                                        <span className="text-[10px] font-mono text-white/20">#{result.id.slice(0, 8)}...</span>
                                    </div>
                                    <h4 className="text-xl font-display text-white group-hover:text-indigo-400 transition-colors">{result._title}</h4>
                    ...
                    <div className="py-20 text-center border border-dashed border-white/10 rounded-sm">
                        <p className="text-white/20 italic font-display text-xl">No se encontraron resultados para "{query}"</p>
                    </div>
                )}
                ...
                {!query && !isLoading && (
                    <div className="py-20 text-center">
                        ...
                        <p className="text-white/20 font-display text-lg">Selecciona una tabla y busca para explorar el ecosistema.</p>
                    </div>
                )}
            </div>
        </div>
